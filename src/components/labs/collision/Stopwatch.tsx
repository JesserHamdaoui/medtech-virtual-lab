...
export default function Stopwatch({ simulation }: StopwatchProps) {
  const timeElapsedProperty = simulation["timeElapsedProperty"];
  const isMovingProperty = simulation["isMovingProperty"];

  const [timeElapsed, setTimeElapsed] = useState<number>(
    timeElapsedProperty.value
  );
  const [isMoving, setIsMoving] = useState<boolean>(isMovingProperty.value);

  const animationFrameRef = useRef<number | undefined>(undefined);

  useEffect(() => {
    const update = () => {
      setTimeElapsed(timeElapsedProperty.value);
      setIsMoving(isMovingProperty.value);
  ...
  }, [timeElapsedProperty, isMovingProperty]);
  ...
  return (
    <div
      className="
        absolute top-5 right-[350px] z-[1000] min-w-[160px]
